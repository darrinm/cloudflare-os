...
describe("auditRows", () => {
  it("keeps every event, newest first, naming the Bot and reading decisions from the data", () => {
    ...
    expect(rows[4]).toMatchObject({ bot: "Scout", type: "delivered", text: "to agent" });
...
describe("filterRows", () => {
  ...
  it("narrows by Bot, kind and text, each optional", () => {
    ...
    expect(filterRows(rows, { bot: "Fixer", type: "", q: "" }).map((r) => r.id)).toEqual([3, 2]);
    ...
    expect(filterRows(rows, { bot: "Fixer", type: "completed", q: "seats" }).map((r) => r.id)).toEqual([2]);
    expect(filterRows(rows, { bot: "Scout", type: "failed", q: "" })).toEqual([]);
  });
});
