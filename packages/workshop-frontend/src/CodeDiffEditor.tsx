...
export default function CodeDiffEditor({
  ...
  if (!filename || (!originalYText && !modifiedYText)) {
    return (
      <div
        className="flex items-center justify-center bg-kumo-base text-[14px] md:text-[13px] leading-[18px] tracking-[-0.25px] text-kumo-subtle"
        style={{ height }}
      >
    ...
    <div ref={containerRef} className="flex min-h-0 overflow-hidden bg-kumo-base" style={{ height }}>
      ...
      >
        <div className="absolute right-3 top-3 flex items-center gap-2" style={{ zIndex: 1 }}>
          ...
          <div
            className="pointer-events-none flex h-7 items-center gap-2 rounded-lg border border-kumo-line bg-kumo-base px-2 font-mono text-[12px] md:text-[11px] leading-4 tracking-[-0.2px] shadow-sm"
            style={{ fontFamily: monoFont }}
          >
            {model.status !== 'Modified' && (
              <span className="text-[11px] md:text-[10px] font-medium text-kumo-subtle">{model.status}</span>
            )}
