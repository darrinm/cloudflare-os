...
describe('WorkspaceOpenErrorPage', () => {
  ...
  it('explains how to recover when access is denied without exposing workspace metadata', async () => {
    ...
    expect(renderedContainer.querySelector('h1')?.textContent).toBe("You don't have access to this project")
    expect(renderedContainer.textContent).toContain('Ask the project owner to grant you access, then try again.')
    ...
    expect(buttons.map(button => button.textContent)).toEqual(['Go to projects', 'Try again'])
  ...
  it('gives a missing workspace a distinct, non-retryable state', async () => {
    ...
    expect(renderedContainer.querySelector('h1')?.textContent).toBe('Project not found')
    expect(renderedContainer.textContent).toContain('The link may be incorrect, or the project may have been deleted.')
    expect([...renderedContainer.querySelectorAll('button')].map(button => button.textContent))
      .toEqual(['Go to projects'])
  })

  it('keeps unexpected failures retryable', async () => {
    const { container: renderedContainer } = await render('unexpected')

    expect(renderedContainer.querySelector('h1')?.textContent).toBe("We couldn't load this project")
    expect(renderedContainer.textContent).toContain('Try again. If the problem continues, return to your projects.')
    expect([...renderedContainer.querySelectorAll('button')].map(button => button.textContent))
      .toEqual(['Go to projects', 'Try again'])
  })
