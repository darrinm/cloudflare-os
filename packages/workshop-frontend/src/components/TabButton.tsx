...
export function TabButton({ active, onClick, children, badgeCount = 0, className = '' }: TabButtonProps) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`relative flex ${heightClassName} cursor-pointer items-center gap-1.5 text-[14px] md:text-[13px] leading-[18px] tracking-[-0.25px] transition-colors ${
        active
    ...
    >
      ...
      {badgeCount > 0 && (
        <span
          className="inline-flex h-[14px] min-w-[14px] items-center justify-center rounded-full bg-kumo-contrast px-1 text-[11px] md:text-[10px] leading-none font-semibold text-kumo-inverse"
