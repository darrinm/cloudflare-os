...
export default function SidebarGadgetRow({
  ...
  return (
    <Link
      to="/workspace/$id"
      params={{ id: gadget.id }}
      className="group flex h-8 items-center gap-2 rounded-lg pl-1.5 pr-1 text-[14px] md:text-[13px] leading-[18px] tracking-[-0.25px] text-kumo-default transition-colors hover:bg-kumo-tint"
      activeProps={{ className: 'flex h-8 items-center gap-2 rounded-lg pl-1.5 pr-1 text-[14px] md:text-[13px] leading-[18px] tracking-[-0.25px] bg-kumo-fill text-kumo-strong font-medium' }}
      onClick={(e) => {
        ...
        <>
          {renaming ? (
            <input
              ...
              className="min-w-0 flex-1 bg-transparent text-[14px] md:text-[13px] leading-[18px] tracking-[-0.25px] outline-none border-b border-kumo-brand text-kumo-default"
