...
class OverseerImpl implements AgentHooks {
  ...
  /**
   * Re-apply a blueprint's files to an existing gadget, in place: how a gadget created from an
   * earlier version of a blueprint takes a later one. Storage, bindings and the workpiece id are
   * kept, and files the blueprint no longer has are left alone. Returns which files changed.
   *
   * This is the only way an existing Bots hub takes new hub code (Bots -> Skills -> "Update Bots").
   * It wrote into the workspace Yjs doc until upstream moved code onto the git store; it is now one
   * commit on the gadget's head, which is both simpler and the same thing every other write does.
   */
  async updateGadgetFromBlueprint(id: WorkpieceId, blueprintId: string)
      : Promise<{updated: string[], unchanged: string[]}> {
    let record = this.getGadgetRecord(id);  // validate it exists
    let {files} = await this.fetchBlueprint(blueprintId);

    let current = await this.readGadgetFiles(id);
    let next = new Map(current);
    let updated: string[] = [];
    let unchanged: string[] = [];
    for (let [file, content] of Object.entries(files)) {
      if (current.get(file) === content) {
        unchanged.push(file);
      } else {
    ...
    if (updated.length > 0) {
      ...
      // Fresh stub per call so the pure whoami() read can be retried across a user-DO reset.
      let owner = () => wrapDoStubForTelemetry(
          this.users.get(this.users.idFromString(ownerId!)), this.logger);
      let ownerProfile = await retryOnDoReset(() => owner().whoami(), this.logger);
      let commitId = await this.gitStore.writeFilesAsCommit(next, {
        // A gadget still pending in a chat has no head yet; its first commit is a root.
        parents: record.commitId !== undefined ? [record.commitId] : [],
        author: commitIdentityForAuthor(ownerProfile),
        message: `Update from blueprint: ${blueprintId}`,
        timestamp: new Date(),
      });
      // Re-read: the awaits above give another writer room to move the head.
      let fresh = this.storage.gadgets.get(id)!;
      fresh.commitId = commitId;
      this.storage.gadgets.put(fresh);
      // Cached workers hold the old code until the loader-cache counter moves.
      this.bumpVersion();
    }
  ...
  async ownedOutputWorkspace(initiator: AiChatAuthorInfo, outputId: string): Promise<string | null> {
    // The initiator's own outputs index, resolved as listAvailableBlueprints resolves their library.
    let userStub = this.users.get(this.users.idFromName(initiator.id));
    return ownedOutput((await userStub.listOutputs()).outputs, outputId)?.workspaceId ?? null;
  }
